...
async function forgotPassword() {
  ...
  try {
    const response = await fetch(
      "/processes/verifyCodeProcess.php?email=" + email
    );
    if (response.ok) {
...
function verifyCode() {
  ...
  request.open("POST", "./processes/verifyCode2Process.php", true);
  request.send(form);
}
...
function resetPassword() {
  ...
  if (type_email && type_password && type_retype_password) {
    ...
    request.open("POST", "./processes/resetPasswordProcess.php", true);
    request.send(form);
  } else {
    document.getElementById("msgToast").classList.remove("hidden");
