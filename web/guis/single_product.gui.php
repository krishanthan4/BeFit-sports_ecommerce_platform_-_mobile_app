    ...
    <div class="lg:grid lg:grid-cols-12 lg:auto-rows-min lg:gap-x-8">
        <div class="lg:col-start-8 lg:col-span-5">
          <div class="flex justify-between">
            <h1 class="text-xl font-medium text-gray-200"><?= $singleProduct["title"] ?></h1>
          </div>
   <!-- reviews divider div -->
...
?>
  ...
  $productImage1 = $productImage_rs->fetch_assoc();
              ...
              ?>
            <div class="bg-[#2b2d30a4] mb-4 w-[60%] h-fit p-4 rounded-md">
            <img draggable="false" src="<?= $productImage1["img_path"]?>" class="w-full rounded-md " alt="<?= $productImage1["product_id"]?> ">
            </div>
        ...
        <div class="lg:col-span-5">
          <form>
            ...
            <h2 class="text-sm font-medium text-gray-400">Color</h2>
  <?php }?>
              <fieldset class="mt-2">
         ...
         include './guis/partials/colorsMap.php';
              ...
              $normalizedColorName = strtolower(str_replace(' ', '', $color["clr_name"])); // Normalize the color name
              $hexColor = $normalizedColorHexMap[$normalizedColorName] ?? "#FFFFFF";
              ?>
                   <label class="-m-0.5 relative p-0.5 rounded-full flex items-center justify-center cursor-pointer focus:outline-none ring-[<?= $hexColor?>]">
                    <input type="radio" name="color-choice" value="<?=  strtolower($color["clr_name"])?>" class="sr-only" aria-labelledby="color-choice-1-label">
                    <span aria-hidden="true" class="h-8 w-8 bg-[<?= $hexColor?>] border border-black border-opacity-10 rounded-full"></span>
                  </label>
              <?php
            ...
            <p class="text-xs my-3 ms-5  bg-<?php if($singleProduct["qty"]>0){
              ...
              ?>red<?php
            }?>-900/50 text-gray-400 w-fit p-1 px-2 rounded-lg"><?php if($singleProduct["qty"]>0){
              ?>In stock<?php
            }else{
              ?>Out Of Stock<?php
            }?></p>
...
<p class="text-xl font-medium text-gray-400">Rs.<?= ($singleProduct["price"])?>.00</p>
...
<div class="  flex justify-between max-w-60 my-4 items-center">
    ...
    <div class="relative flex items-center">
        <button  <?php if($singleProduct["qty"]==0){
              ?>disabled<?php }else{}?> type="button" id="decrement-button" data-input-counter-decrement="counter-input" class="flex-shrink-0 bg-[#3a3d41] hover:bg-[#28292c]  inline-flex items-center justify-center rounded-md h-6 w-6 ">
            <svg class="w-2.5 h-2.5 text-gray-400 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 1h16"/>
            </svg>
        </button>
        <input  <?php if($singleProduct["qty"]==0){
              ?>disabled<?php }else{}?>  type="text" id="counter-input" data-input-counter data-input-counter data-input-counter-min="1" data-input-counter-max="<?= $singleProduct["qty"]?>" class="flex-shrink-0 text-gray-400 border-0 bg-transparent text-sm font-normal focus:outline-none focus:ring-0 max-w-[2.5rem] text-center" placeholder="" value="1" required />
        <button  <?php if($singleProduct["qty"]==0){
              ?>disabled<?php }else{}?>  type="button" id="increment-button" data-input-counter-increment="counter-input" class="flex-shrink-0 bg-[#3a3d41] hover:bg-[#28292c] inline-flex items-center justify-center rounded-md h-6 w-6 ">
            <svg class="w-2.5 h-2.5 text-gray-400 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
...
<!-- qty counter -->
   ...
   <button  <?php if($singleProduct["qty"]==0){
              ?>disabled<?php }else{}?>  type="button" onclick="buyNow('<?= $singleProduct['id'] ?>');" class="mt-8 w-full bg-orange-700/70 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-orange-900/70 ">Buy Now</button>
   <button  <?php if($singleProduct["qty"]==0){
              ?>disabled<?php }else{}?>  type="button" onclick="addToCart('<?= $singleProduct['id'] ?>');" class="mt-8 w-full bg-blue-700/70 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-blue-900/70 ">Add to cart</button>
   </div>
   
  
          <!-- Policies -->
          <section aria-labelledby="policies-heading" class="mt-10">
              <div class="bg-[#222427] rounded-lg p-6 text-center">
                <dt>
                  ...
                  <span class="mt-4 text-sm font-medium text-gray-400"> International delivery </span>
                </dt>
...
require_once "./guis/partials/singleproductView/reviews.php";
require_once "./guis/partials/alert.php";
require_once "./guis/partials/singleproductView/relatedProducts.php";
