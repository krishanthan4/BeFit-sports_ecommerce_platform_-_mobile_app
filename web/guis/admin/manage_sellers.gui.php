...
<div class="w-full mx-4  my-4 flex flex-row items-center">
        <p class="sm:text-xl sm:text-center text-base md:ms-6">Manage Sellers</p>
        <!-- search bar -->
            <div id="searchForm"  class="flex xl:w-[70%] lg:max-w-[50%] lg:min-w-[40%] sm:w-[70%] gap-2 items-center  mx-10">   
                    <input type="text" id="searchInput" placeholder="Search by name or email"  class="bg-[#2e3035] text-gray-400 border border-[#383 93f]  text-sm rounded-lg w-full p-2 outline-none" required />
            </div>
    </div>
    ...
    <div class="-my-2 overflow-x-auto sm:-mx-4 md:-mx-6 lg:mx-2">
        <div class="py-1 align-middle inline-block min-w-full sm:px-4 md:px-6 lg:px-8">
            <div class="shadow overflow-hidden  sm:rounded-lg">
                <table id="manageSellersTable" class="lg:min-w-[90%] sm:md-w-[60%] md:min-w-full overflow-x-scroll divide-y divide-[#3e3e46]">
                    <!-- Table Headers -->
                    <thead class="bg-[#26262b]">
                        <tr>
                            ...
                            <th scope="col" class="px-2 md:px-3 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Total Products </th>
                            <th scope="col" class="px-2 md:px-3 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider"> Action </th>
                        </tr>
                    </thead>
                    <tbody id="sellerTableBody" class="bg-[#26262b]/10 divide-y divide-[#3e3e46]">
<?php 
$AllSellers_rs = Database::search("SELECT COUNT(DISTINCT `user_email`)as total FROM `product`;");
$AllSellers = $AllSellers_rs->fetch_assoc();
$totalCount = $AllSellers['total'];
$itemsPerPage = 7;
$baseurl='/manageSellers?id=';
$totalPages = ceil($totalCount / $itemsPerPage);

if($AllSellers_rs->num_rows){
...
$manageResult = Database::search("SELECT `user`.*, COUNT(`product`.`user_email`) AS num_products
FROM `user`
...
if($manageResult->num_rows!==0){
    for ($i=0; $i < $manageResult->num_rows; $i++) { 
        $manageSellers= $manageResult->fetch_assoc();
    ?>
                            ...
                            <td class="px-3 py-2 whitespace-nowrap">
                                ...
                                <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <?php
    $ManageUserProfile = Database::search("SELECT `path` FROM profile_img WHERE user_email='".$manageSellers["email"]."'");
    ...
    ?>
                                ...
                                <?php 
                                if($manageSellers["fname"]==!null || $manageSellers["lname"]){
    ?>
                                  <td class="px-3 py-2 whitespace-nowrap">
                                    <div class="text-sm text-gray-300"><?=  $manageSellers["fname"] ." ".$manageSellers["lname"]?> </div> 
                                </td>
    <?php
                                }else{
                                    ?>
    ...
    <td id="email" class="px-3 py-2 whitespace-nowrap text-sm text-gray-300"><?= $manageSellers["email"]?></td>
    
    
    
                              
    
                                <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-300"><?= $manageSellers["mobile"]?></td>
                                <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-300"><?= $manageSellers["joined_date"]?></td>
                                <td class="px-3 py-2 whitespace-nowrap text-center text-sm text-gray-300"><?= $manageSellers["num_products"]?></td>
                                
                                <td class="px-3 py-2 whitespace-nowrap text-right text-sm font-medium flex items-center justify-center">
                               
                               
                                <button id="blockbutton-<?= $manageSellers["email"]?>"
        onclick="toggleSellerStatus('<?= $manageSellers['email'] ?>')"
        data-initial-status="<?= $manageSellers['status_status_id'] ?>"
        class="<?php if($manageSellers["status_status_id"]==1){?>hover:bg-red-200 text-red-700 border-red-600 <?php }else{?>hover:bg-green-200 text-green-700 border-green-600<?php }?> inline-flex items-center px-6 py-2 border text-[16px] font-bold rounded ">
    <?php if($manageSellers["status_status_id"]==1){?>
        block
    <?php }else{?>
        unblock
    <?php } ?>
    ...
    ?> <!-- More people... -->
          ...
          <div id="emptyManageSellers" class="col-span-12 hidden lg:col-span-6 flex flex-col justify-center">
<img src="./public/images/noitems.jpg" alt="" class="h-[290px] bg-center w-fit mx-auto bg-contain bg-no-repeat my-2">
    ...
    ?>
    <script>window.location.href = "/manageSellers?id=1"</script><?php
}
...
// Assuming the base URL and totalPages are already set
$currentPage = isset($_GET['id']) ? intval($_GET['id']) : 1;
$currentPage = max(1, min($currentPage, $totalPages));
$parsedJs = "manageSellersPagination"; 
...
<script src="./public/js/manageSellers.js"></script>
</div></body></html>
<?php } else{?>
