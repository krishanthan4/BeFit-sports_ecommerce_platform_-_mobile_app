...
<?php function relatedProduct($title,$category_id){
  ?>
  <section aria-labelledby="related-heading" class="mt-16 px-9 sm:mt-24">
      <h2 id="related-heading" class="text-lg font-medium text-gray-200"><?= $title ?></h2>
...
?>
   <div class=" grid grid-cols-2 gap-6 my-10 xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3">
  <?php
      for ($e=0; $e < $relatedProduct_rs->num_rows; $e++) { 
        $relatedProduct = $relatedProduct_rs->fetch_assoc();
?>
    <div class="group flex flex-col justify-between bg-[#28292c] rounded-md relative p-4 sm:p-6">
...
?>
            ...
            <img draggable="false" src="<?= $relatedProductImage["img_path"]?>" alt="<?= $relatedProduct["title"]?>" class="w-full min-h-40 min-w-40 max-w-50 max-h-50  h-full object-center object-cover">
          </div>
