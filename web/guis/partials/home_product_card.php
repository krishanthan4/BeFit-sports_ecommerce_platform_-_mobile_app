...
$pageCount_rs = Database::search("SELECT COUNT(id) AS total FROM `product`");
if ($pageCount_rs->num_rows > 0) {
  ...
  $product_result = Database::search("SELECT DISTINCT `product`.`id`,`product`.`price`,`product`.`title`,`product`.`datetime_added` FROM `product` INNER JOIN `condition` ON `product`.`condition_condition_id`= `condition`.`condition_id` WHERE `status_status_id`='1' ORDER BY `product`.`datetime_added` DESC LIMIT $from,$to ");
...
<div class="group flex flex-col justify-between  relative p-2 border rounded-md border-[#1d1e20] bg-[#26282bee] sm:p-6">
          ...
          if ($productImage_rs->num_rows > 0) {
            ...
            <img src="<?= $productImage["img_path"] ?>" alt="<?= $product["title"] ?>"
              class="w-full min-h-40 min-w-40 max-w-50 max-h-50  h-full object-center object-cover">
          <?php }
          ?>
        ...
        <div class="pt-10 pb-4 aspect-w-1 aspect-h-1 max-h-40 min-h-20 text-center">
          ...
          <p class="mt-4 text-base font-medium text-gray-400">Rs.
            <?= $product["price"] ?>.00
          </p>
