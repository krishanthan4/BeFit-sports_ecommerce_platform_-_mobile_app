...
if ($_GET["query"]) {
  ...
  if ($search_rs->num_rows !== 0) {
    ?>
    <div class=" gap-2 grid grid-cols-2 sm:mx-0 md:grid-cols-3 lg:grid-cols-6 py-6 px-4">
      <?php
      for ($e = 0; $e < $search_rs->num_rows; $e++) {
        $search = $search_rs->fetch_assoc();
        ?>

        <div class="group flex flex-col justify-between  relative p-2 bg-[#25262b] rounded-md  sm:p-6 my-6">
          ...
          <div class="pt-10 pb-4 aspect-w-1 aspect-h-1 max-h-40 min-h-20 text-center">
            <h3 class="text-sm font-medium text-gray-400">
              <a href="/singleProduct?product_id=<?= $search["id"] ?>">
                ...
                <?php
                if (strlen($search["title"]) > 20) {

                  echo substr($search["title"], 0, 20) . "...";
                } else {
                  echo $search["title"];
                }
                ?>
    ...
    <p class="text-gray-600 px-3 ms-11 mt-4 font-sans">Did you mean <span class="text-red-400">"<?= $query ?>"</span>? ...
    </p>
    ...
    <?php }
  require_once "./guis/partials/footer.php";
